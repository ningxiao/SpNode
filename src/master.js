...
const StrutsAnalyze = require('./struts/StrutsAnalyze');
//agent进程实现类似redis功能并且监听文件改变修改重启工作进程
const agent = cp.fork(path.join(__dirname, "agent/index.js"), ['--port', config.agent.port], {
    env: {
...
const createWorker = (sum) => {
    cluster.setupMaster({
        exec: path.join(__dirname, 'server/worker.js'),
        args: [config.port, config.agent.port],
        silent: false
    });
    for (let i = 0; i < sum; i++) {
        cluster.fork({
...
const upaction = ()=>{
    let worker;
    console.log("************************更新action配置************************");
    config.action = StrutsAnalyze();
...
agent.on('message', (msg) => {
    switch (msg.head) {
        ...
        case "agent-worker-update": //agent发送命令重启工作进程
            reload();
            break;
        case "agent-worker-action": //agent发送命令重启工作进程
            upaction();
            break;
        default:
            break;
